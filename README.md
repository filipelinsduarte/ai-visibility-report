...
A command-line tool for marketing agencies to generate AI visibility prospect reports. Pull live data from the AI Peekaboo API, process it, and produce a self-contained HTML dashboard showing a brand's visibility across 5 AI models. Action recommendations are auto-generated by any LLM you already have access to.
...
- **Actions** — 6 data-driven recommendations generated by your LLM, specific to that brand's data
...
- **An API key for any major LLM** — Anthropic, OpenAI, Google Gemini, Groq, Mistral, OpenRouter, or any OpenAI-compatible endpoint
- **Python 3.8+**
...
3. Copy the example config and fill in your keys and brand IDs:
   ...
   ```
   Open `config.json` and set your AI Peekaboo API key, your LLM provider + API key, and brand details.
...
The 6 action recommendations in each report are generated by whichever LLM you configure — the same prompt is sent to any provider. They are not templates.

The model receives the brand's real visibility percentages, the exact domains that appear most in citations, the competitor list with their relative visibility scores, sentiment distribution, and content type breakdown. Every recommendation is specific to what the data shows: if a competitor's knowledge base is cited 40 times and yours is cited 3 times, the action will reference those domains and counts directly.
...
| Provider | `llm_provider` | Default model | Key format |
|---|---|---|---|
...
| OpenRouter | `openrouter` | `anthropic/claude-sonnet-4-6` | `sk-or-...` |
| Mistral | `mistral` | `mistral-large-latest` | API key |
| Any OpenAI-compatible | `openai` | — | set `llm_base_url` too |
